...
robots = []
pickups = []
#music and injury playlist [Jon - 4/23/10]
...
while True:
    ...
    background = pygame.transform.scale(pygame.image.load('background.bmp'),(WINDOWWIDTH,WINDOWHEIGHT))
    ...
    if not game_start:
        ...
        windowSurface.set_colorkey(BLACK)
    ...
    if game_start:
        ...
        if not game_pause:
            ...
            if robotcount >= robotspawntime:
                ...
                if side == 0:
                    spawnx = random.randint(0,WINDOWWIDTH)
                    spawny = -50
                if side == 1:
                    spawnx = WINDOWWIDTH + 50
                    spawny = random.randint(0,WINDOWHEIGHT)
                if side == 2:
                    spawnx = random.randint(0,WINDOWWIDTH)
                    spawny = WINDOWHEIGHT + 50
                if side == 3:
                    spawnx = -50
                    spawny = random.randint(0,WINDOWHEIGHT)
        ...
        windowSurface.blit(areaSurface, pygame.Rect(0,0,WINDOWWIDTH, WINDOWHEIGHT))
