...
WINDOWWIDTH = 1200
WINDOWHEIGHT = 800
#$ areaSurface = pygame.Surface((WINDOWWIDTH, WINDOWHEIGHT), 0, 32)
...
#$
    def draw(self):
        if approxInSurface(self.xpos,self.ypos):
            pygame.draw.circle(windowSurface,WHITE,(self.xpos,self.ypos),self.size,0)
...
#$
    def draw(self):
        if approxInSurface(self.xpos,self.ypos):
            pygame.draw.circle(windowSurface,RED,(self.xpos,self.ypos),self.size,0)
...
class LazergunBullet(Bullet):
    ...
    def draw(self):
        pygame.draw.rect(windowSurface, RED, (self.xpos, self.ypos, 60, 10))
...
class Explosion(object):
    ...
    def draw(self):
        ...
        if approxInSurface(rect.left,rect.top):
...
def approxInSurface(x,y):
    if x > camera.getLeft() and x < camera.getRight() and y > camera.getTop() and y < camera.getBottom():
        return True
...
while True:
    ...
    background = pygame.transform.scale(pygame.image.load('temp_background.bmp'),(AREAWIDTH,AREAHEIGHT))
    #$if approxInSurface(self.xpos,self.ypos):
    windowSurface.blit(background,pygame.Rect(0,0,WINDOWWIDTH,WINDOWHEIGHT))
...
#$
            if approxInSurface(pickup_rect.left,pickup_rect.top):
                windowSurface.blit(pickup.image,pickup_rect)
        for bullet in bullets:
            bullet.draw()
        for bullet in robobullets:
            bullet.draw()
        ...
        for player in players:
            ...
            current_image.set_colorkey(BLACK)
#$
            if approxInSurface(player_rect.left,player_rect.top):
                windowSurface.blit(current_image,player_rect)
        for robot in robots:
            ...
            current_image.set_colorkey(BLACK)
#$
            if approxInSurface(robot_rect.left,robot_rect.top):
                windowSurface.blit(current_image,robot_rect)
        for explosion in explosions:
            explosion.draw()
